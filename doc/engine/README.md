...
Список Phaser-сцен и точка входа приложения: **[../core/SCENES_REFERENCE.md](../core/SCENES_REFERENCE.md)**.
...
Данные баланса (работы, жильё, демо-сейв, навыки UI, образование, базовые месячные расходы): **`src/balance/`** (см. `index.js`). Миграции версий сохранений встроены в **`PersistenceSystem`**.

```
src/
├── balance/                          # Баланс и статический контент (не только ECS)
│   ├── index.js
│   ├── default-save.js
│   ├── career-jobs.js
│   ├── housing-levels.js
...
└── game-state.js                     # Legacy API, merge defaults, часть игровых событий
```
