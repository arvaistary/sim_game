...
**Статус:** В процессе (~65%) — поиск работы и вакансии в ветке авг 2026

**Задачи:**
- [x] Ручное переключение между работами (смена карьеры)
- [x] Поиск работы: каналы, недельный лимит, сохранённый список вакансий (`FindWorkModal`, `playerState.jobSearch`)
- [x] Система требований для вакансий (профессионализм, образование, вещи)
- [ ] Интеграция вещей с магазином/инвентарём (компьютер для интернет-поиска)
- [ ] Специальные рабочие события по профессии
- [x] Архитектура job search: domain-типы/правила вынесены из config, orchestration — в `useJobSearch`
