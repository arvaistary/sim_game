...
| **6. Система смерти и концовок** | ⚠️ В процессе | ~45% |
| **7. Случайные события** | 🔄 В процессе | ~70% |
...
- ✅ План дня с единым подтверждением «Прожить день»
- ✅ Фаза работы через общий `useWorkShiftOptions` на дашборде и странице карьеры
- ✅ Фаза восстановления
- ✅ Система шкал персонажа (6 шкал): Голод, Энергия, Стресс, Настроение, Здоровье, Физическая форма
- ✅ Система времени и возраста (часовая модель, бюджеты производны от `totalHours`)
- ✅ Нейтральное закрытие остатка суток и domain hooks границ дня/недели/месяца/года
...
**Статус:** В процессе (~45%)

**Что реализовано:**
- ✅ Пять причин смерти и приоритет правил: болезнь, естественная смерть, авария, депрессия, истощение
- ✅ Дневной low-mood streak и идемпотентное завершение жизни
- ✅ Финальный `LifeSummary`: возраст, время, финансы, карьера, топ навыков, семья, жильё и доступные метрики достижений/хобби
- ✅ Типы концовок и оценка жизни в 1–5 звёзд
- ✅ Сохранение итогового состояния в `GameWorld`, bridge, Pinia и JSON
- ✅ Отдельная `MetaProgression`-модель: статистика игрока, счётчик жизней, причины смерти, best-метрики и New Game+ transfer
- ✅ Финальный экран, JSON-экспорт и New Game+ через существующий init flow
- ✅ Redirect завершённой игры и блокировка игровых команд после смерти

**Что не реализовано:**
- ⚠️ Модификаторы продолжительности жизни из здоровья, комфорта и семьи
- ⚠️ Полные модели семьи, транспорта, хобби и достижений; producers достижений/knowledge и UI выбора transfer пока не реализованы
- ⚠️ Текущий New Game+ transfer — тестовый runtime-default; требуется отдельная доработка баланса в пользу реализма без сильного преимущества. Возможный путь — стартовое место рождения/происхождение с умеренным влиянием на деньги, инвентарь и навыки
- ⚠️ Экспорт красивого изображения, портрет и QR-код
...
- ✅ Канонический формат choice (`id`/`text`/typed effects) + resolver
- ✅ Подключение к `DayEndHooks`: work (≤1/день), micro, weekly, monthly+finance, yearly, age 30/40/50/60
- ✅ `RandomSource` порт (domain) + `MathRandomSource` (infrastructure)
- ✅ Cooldown через `cooldownByEventId`
- ✅ Увольнение за недоработку часов (`job_dismissal` → `endCareerWork`)

**Что не реализовано:**
- ⚠️ Полный список 25 событий GDD (контент частично)
- ⚠️ Micro chance modifiers (risk/skill/state) — follow-up
- ⚠️ Childhood events — follow-up
...
- ✅ Meta-хранилище `unlockedAchievements`, сохраняемое между жизнями
