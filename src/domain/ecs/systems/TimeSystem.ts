...
import { MICRO_EVENT_BY_ACTION, buildMicroQueuedEvent } from '../../balance/game-events'
...
type PeriodicCallback = (...args: number[]) => void
...
export class TimeSystem {
  ...
  readonly HOURS_IN_WEEK = 168
  readonly WEEKS_IN_MONTH = 4
  readonly MONTHS_IN_YEAR = 12
  readonly DAYS_IN_AGE_YEAR = 360
  ...
  init(world: ECSWorld): void {
    this.world = world
    const time = this.world.getComponent(PLAYER_ENTITY, TIME_COMPONENT) as TimeComponent | null
    if (time) {
  ...
  getDayHoursRemaining(): number {
    const timeComponent = this.world?.getComponent(PLAYER_ENTITY, TIME_COMPONENT) as TimeComponent | null
   ...
   */
  normalizeTimeComponent(timeComponent: TimeComponent): void {
  ...
  advanceHours(hours = 1, options: AdvanceOptions = {}): AdvanceResult {
    ...
    const timeComponent = this.world.getComponent(playerId, TIME_COMPONENT) as TimeComponent | null
  ...
  maybeTriggerMicroEvent(actionType = 'default', options: AdvanceOptions = {}): unknown {
    const playerId = PLAYER_ENTITY
    const time = this.world.getComponent(playerId, TIME_COMPONENT) as TimeComponent | null
  ...
  _buildMicroEvent(def: { id: string; title: string; description: string; baseChance: number }, actionType: string): unknown {
    const time = this.world.getComponent(PLAYER_ENTITY, TIME_COMPONENT) as TimeComponent
