...
/**
 * Валидатор для действий
 * Проверяет действие на соответствие схеме и возвращает ошибки если есть
 */
export class ActionValidator {
   ...
   * @returns true если валидно, false если есть ошибки
   */
  validate(action: unknown): action is ValidatedBalanceAction {
    this.errors = []
    ...
    if (!result.success) {
      result.error.issues.forEach((issue) => {
        this.errors.push(`${issue.path.join('.')}: ${issue.message}`)
      })
      return false
    }
   ...
   */
  static validateUniqueIds(actions: BalanceAction[]): {
    valid: boolean
    duplicates: string[]
  } {
   ...
   * @returns Объект с результатом проверки
   */
  static validateRequiredFields(actions: BalanceAction[]): {
    valid: boolean
   ...
   */
  static validateCatalog(actions: BalanceAction[]): {
    valid: boolean
    ...
    const validator = new ActionValidator()
    const schemaResult = validator.validateArray(actions)
    const uniqueIdsResult = ActionValidator.validateUniqueIds(actions)
    const requiredFieldsResult = ActionValidator.validateRequiredFields(actions)
...
/**
 * Функция для валидации каталога действий (удобная для использования)
 */
export function validateActionCatalog(actions: BalanceAction[]): ReturnType<typeof ActionValidator.validateCatalog> {
  return ActionValidator.validateCatalog(actions)
}
