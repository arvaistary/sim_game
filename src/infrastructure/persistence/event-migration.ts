...
export class EventMigration {
   ...
   */
  migrateQueueItem(legacy: LegacyEventQueueItem, totalHours: number): CanonicalEventQueueItem {
    return {
      ...legacy,
   ...
   */
  migrateHistoryEntry(legacy: LegacyEventHistoryEntry, totalHours: number): CanonicalEventHistoryEntry {
    return {
   ...
   */
  migrateSave(payload: Record<string, unknown>): MigrationResult {
    ...
    try {
      ...
      if (eventQueue && eventQueue.pendingEvents) {
        ...
        eventQueue.pendingEvents = pendingEvents.map(event => 
          this.migrateQueueItem(event, totalHours)
        )
      ...
      if (eventHistory && eventHistory.events) {
        ...
        eventHistory.events = events.map(event => 
          this.migrateHistoryEntry(event, totalHours)
        )
   ...
   */
  needsMigration(payload: Record<string, unknown>): boolean {
   ...
   */
  validateQueueItem(item: unknown): item is CanonicalEventQueueItem {
   ...
   * @returns true если валидно
   */
  validateHistoryEntry(entry: unknown): entry is CanonicalEventHistoryEntry {
