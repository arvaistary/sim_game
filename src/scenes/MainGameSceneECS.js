...
/**
 * MainGameScene с полной поддержкой ECS
...
export class MainGameSceneECS extends Phaser.Scene {
  ...
  handleResize(gameSize) {
    ...
    this.actionCard.setPosition(isDesktop ? (480 - 480) / 2 : 20, h - 120);
  ...
  ensureEventQueue(delay = 0) {
    this.time.delayedCall(delay, () => {
      ...
      const pendingEvents = eventQueueSystem.getPendingEvents(playerId);
