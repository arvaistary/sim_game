...
function formatEffectLine([key, effect], maxLevel) {
  const label = EFFECT_LABELS[key] || humanizeKey(key);
  const value = typeof effect === 'function' ? effect(maxLevel) : effect;
  return `• ${label}: ${formatEffectValue(key, value)}`;
}

export function buildSkillTooltipText(skill) {
  if (!skill) return '';

  const maxLevel = skill.maxLevel ?? 10;
  const effectLines = Object.entries(skill.effects || {}).map((entry) => formatEffectLine(entry, maxLevel));
  const milestoneLines = Object.entries(skill.milestones || {}).map(
    ([level, milestone]) => `• ${level} ур.: ${milestone.description}`,
  );
